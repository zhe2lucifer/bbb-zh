...
static int cache_change_flag = 0;

extern char * package_buf;
extern int package_len;
extern uint8_t net_download_finish;
static net_cainfo_t net_cainfo;
static unsigned char *net_capath = NULL;
...
int change_net_cache(char * net_buf, int buf_size)
{
	int ret = -1;
	
	if ((net_buf == NULL) || (buf_size == 0))
	{
		SL_ERR("%s parameters invalid.\n", __FUNCTION__);
		return ret;
	}

	pthread_mutex_lock(&net_cache.cache_mutex);
	memset(&net_cache, 0, sizeof(net_cache_t));
	net_cache.cache_buff = net_buf;
	net_cache.cache_size = buf_size;
	pthread_mutex_unlock(&net_cache.cache_mutex);

	return 0;
}


void set_cache_change_flag(int flag)
{
	cache_change_flag = flag;
}


int get_cache_change_flag(void)
{
	return cache_change_flag;
}
...
static size_t write_data(void *ptr, size_t size, size_t nmemb, void *stream)
{
	...
	/* for one shot mode, should indicate app the progress status */
	desc = (alislupg_desc_t *)net_file_get_desc();
	if ((desc != NULL) && (desc->method == ONE_SHOT))
	{
		total_size = url_get_file_size();
		if (total_size != 0)
		{
			if (nci->unread_data_len >= total_size)
			{
				cur_percent = 100;
				net_download_finish = TRUE;
			}
			else
			{
				cur_percent = (nci->unread_data_len) / (total_size / 100);
			}
			...
			SL_DBG("url read data percent %d\n", __FUNCTION__, desc->percent);
		}
	}
...
static int get_cainfo(void)
{
	...
	if (!net_capath)
		return ret;
#if 0 // Use default path due to cpptest rule
	char *upgCAPath = getenv("UPG_CA_PATH");

	if (!upgCAPath)  /* default path */
	{
#endif
		if ((net_cainfo.pCertFile = (char*)calloc(strlen((char*)net_capath)+16, sizeof(char))))
			strcpy(net_cainfo.pCertFile, (char*)net_capath);
		else
		{
		...
		if ((net_cainfo.pCACertFile = (char*)calloc(strlen((char*)net_capath)+16, sizeof(char))))
			strcpy(net_cainfo.pCACertFile, (char*)net_capath);
		else
		{
		...
		if ((net_cainfo.pKeyName = (char*)calloc(strlen((char*)net_capath)+16, sizeof(char))))
			strcpy(net_cainfo.pKeyName, (char*)net_capath);
		else
		{
...
pthread_t url_open(const char *url)
{
	...
	if (get_cache_change_flag() == 1)
	{
		ret = change_net_cache(package_buf, package_len);
		if (ret < 0)
		{
			SL_DBG("change net cache fail\n");
			return 0;
		}
	}
	else
...
void net_set_ca_path(char *ca_path)
{
	SL_DBG("CA_PATH = %s\n", ca_path);
	net_capath = (unsigned char *)ca_path;
}
